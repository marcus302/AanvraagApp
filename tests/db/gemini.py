...
async def init_db_with_gemini():
    ...
    async with async_session_maker() as session:
        result = await session.execute(select(models.Listing))
    ...
    async def process_listing_field_data(listing):
        async with async_session_maker() as task_session:
            ...
            updated_listing = await parse_field_data_from_listing(
                listing_with_websites, task_session
            )
            task_session.add(updated_listing)
            await task_session.commit()

    field_data_tasks = [
        process_listing_field_data(listing) for listing in random_listings
    ]
    ...
    async def process_client_field_data(client):
        async with async_session_maker() as task_session:
            ...
            updated_client = await parse_field_data_from_client(
                client_with_websites, task_session
            )
            task_session.add(updated_client)
            await task_session.commit()

    client_field_data_tasks = [
        process_client_field_data(client) for client in all_clients
    ]
